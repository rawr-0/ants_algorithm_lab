...
class Window():
    ...
    def __draw_way_graph(self):
        ...
        way, self.waylen = self.method(self.graph, 50, self.startp, self.pgraph, self.in5.get(), self.in4.get(),self.__update_wei)
        self.way = way
        point = list(way)[0]
        for cnode in way:
            if cnode != point:
    ...
    def __draw_param(self):
        ...
        self.l4 = tk.Label(frame,text="Значимость феромона")
        self.in4 = tk.Entry(frame,width=5)
        self.in4.insert(0,"1")
        self.l4.grid(row=2)
        self.in4.grid(row=3)
        self.l5 = tk.Label(frame,text="Количество феромона")
        self.in5 = tk.Entry(frame,width=5)
        self.in5.insert(0,"100")
        self.l5.grid(row=4)
        self.in5.grid(row=5)
        self.l6 = tk.Label(frame,text="Интенсивность испарения")
        self.in6 = tk.Entry(frame,width=5)
        self.l6.grid(row=6)
        self.in6.insert(0,"0.2")
        self.in6.grid(row=7)

    def __set_weight(self):
        n1 = list(self.graph.nodes())[int(self.in1.get())]
        n2 = list(self.graph.nodes())[int(self.in2.get())]
        self.graph[n1][n2]["weight"] = float(self.in3.get())
        self.__table_ins()
        self.__draw_way_graph()
    ...
    def __clear_wei(self):
        for i in self.pgraph:
            for o in self.pgraph[i].items():
                o[1]["weight"] = 0
    
    def __update_wei(self):
        for i in self.pgraph:
            for o in self.pgraph[i].items():
                if o[1]["weight"] - float(self.in6.get()) < 0:
                    o[1]["weight"] = 0
                else:
                    o[1]["weight"] -= float(self.in6.get())
